	...
	PF_OutData		*out_data)
{
	...
	switch (in_data->appl_id){
		...
		case 'FXTC':
			...
			{
				if (in_data->version.major == PF_AE130_PLUG_IN_VERSION && in_data->version.minor >= PF_AE130_PLUG_IN_SUBVERS) {
					PF_SPRINTF(out_data->return_msg, "Running in After Effects CC 2014 (13.0) or later.");
				...
				} else if (in_data->version.major == PF_AE105_PLUG_IN_VERSION && in_data->version.minor == PF_AE105_PLUG_IN_SUBVERS) {
					PF_SPRINTF(out_data->return_msg, "Running in After Effects CS5.5.");

				} else if (in_data->version.major == PF_AE100_PLUG_IN_VERSION && in_data->version.minor == PF_AE100_PLUG_IN_SUBVERS) {
