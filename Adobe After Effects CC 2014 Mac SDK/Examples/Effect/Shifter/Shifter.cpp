			...
			PF_LayerDef		*output )
{
	...
	S_pica_suiteP 	= in_data->pica_basicP;
	...
	PF_Pixel 	*outP)
{
	...
	if (S_pica_suiteP){
		AEGP_SuiteHandler suites(S_pica_suiteP);

		// Resample original image at offset point.

		new_xFi = ((A_long)xL << 16) + siP->x_offF;
		new_yFi = ((A_long)yL << 16) + siP->y_offF;
	...
	PF_LayerDef		*output )
{
	...
	*/
	...
	si.x_offF = ((long)inputP->width << 15) -	// 1/2 width as Fixed 
		params[SHIFT_DISPLACE]->u.td.x_value;

	si.y_offF = ((long)inputP->height << 15) -	// 1/2 height as Fixed 
		...
		if (infoP){
			...
			if (!err){
				...
				if (!err){
					...
					infoP->x_offF 		= (widthF / 2) - displace_param.u.td.x_value;

					infoP->y_offF 		= (heightF / 2) - displace_param.u.td.y_value;
