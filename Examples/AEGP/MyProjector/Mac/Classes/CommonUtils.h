...
/// 获得 Layer 的名字, 使用完该函数后, 需要手动 delete 删除返回的数据
extern A_UTF16Char * obtainLayerName(AEGP_LayerH layer);
...
extern void obtainInitAnchorPointData(AEGP_LayerH layerH, AEGP_TwoDVal * pValue);

/// 获得初始化的 AnchorPoint 值, 以百分比为单位
extern void obtainInitAnchorPointDataWithPercent(AEGP_LayerH layerH, AEGP_TwoDVal * pValue);

/// 获得初始化的 Position, 以点为单位
extern void obtainInitPositionData(AEGP_LayerH layerH, AEGP_TwoDVal * pValue);

/// 获得初始化的 Scale, 以百分比为单位, 100% 原始大小
extern void obtainInitScaleData(AEGP_LayerH layerH, AEGP_TwoDVal * pValue);

/// 获得初始化的 Rotation, 以度为单位
extern void obtainInitRotationData(AEGP_LayerH layerH, AEGP_OneDVal * pValue);

/// 获得初始化的 Opacity, 以百分比为单位, 100% 不透明, 0% 全透明
extern void obtainInitOpacityData(AEGP_LayerH layerH, AEGP_OneDVal * pValue);

/// 获得初始化的 ZOrder
extern void obtainInitZOrderData(AEGP_LayerH layerH, AEGP_OneDVal * pValue);

// ]]]]

// [[[[

/// 当前选中 Layer - 获得初始化的 AnchorPoint 值, 以像素为单位, 以左下角为坐标原点
extern void obtainInitAnchorPointData(AEGP_TwoDVal * pValue);

/// 当前选中 Layer - 获得初始化的 AnchorPoint 值, 以百分比为单位
extern void obtainInitAnchorPointDataWithPercent(AEGP_TwoDVal * pValue);

/// 当前选中 Layer - 获得初始化的 Position, 以点为单位
extern void obtainInitPositionData(AEGP_TwoDVal * pValue);

/// 当前选中 Layer - 获得初始化的 Scale, 以百分比为单位, 100% 原始大小
extern void obtainInitScaleData(AEGP_TwoDVal * pValue);

/// 当前选中 Layer - 获得初始化的 Rotation, 以度为单位
extern void obtainInitRotationData(AEGP_OneDVal * pValue);

/// 当前选中 Layer - 获得初始化的 Opacity, 以百分比为单位, 100% 不透明, 0% 全透明
...
extern void obtainKeyframesAnchorPointData(AEGP_LayerH layer, AEGP_TwoDVal ** ppValues, float ** ppTimes, A_long * pNumkfs);

/// 获得 AnchorPoint 的动画数据, 百分比为单位, 以图片的左下角为原点
extern void obtainKeyframesAnchorPointDataWithPercent(AEGP_LayerH layer, AEGP_TwoDVal ** ppValues, float ** ppTimes, A_long * pNumkfs);

/// 获得 Position 的动画数据
extern void obtainKeyframesPositionData(AEGP_LayerH layer, AEGP_TwoDVal ** ppValues, float ** ppTimes, A_long * pNumkfs);

/// 获得 Scale 的动画数据
extern void obtainKeyframesScaleData(AEGP_LayerH layer, AEGP_TwoDVal ** ppValues, float ** ppTimes, A_long * pNumkfs);

/// 获得 Rotation 的动画数据
extern void obtainKeyframesRotationData(AEGP_LayerH layer, AEGP_OneDVal ** ppValues, float ** ppTimes, A_long * pNumkfs);

/// 获得 Opacity 的动画数据
extern void obtainKeyframesOpacityData(AEGP_LayerH layer, AEGP_OneDVal ** ppValues, float ** ppTimes, A_long * pNumkfs);
...
/**
 * 当前选中 Layer - 获得 AnchorPoint 的动画数据, 以像素为单位, 以图片的左下角为原点
...
/**
 * 当前选中 Layer - 获得 AnchorPoint 的动画数据, 百分比为单位, 以图片的左下角为原点
 * 注意: 使用完成后, 删除 *ppValues 和 *ppTimes 指向的内存数据
